...
class GuideService:
    ...
    def _build_medication_prompt(self, health_data: dict) -> str:
        ...
        4. 'general_cautions': 일반적인 복약 주의사항을 리스트로 3줄만 작성하세요.
    ...
    def _build_disease_prompt(self, health_data: dict) -> str:
        ...
        2. 'name' 작성 규칙:
           - 질환(disease): 질환 명칭만 작성 (예: 고혈압, 당뇨병)
           - 알러지(allergy): 'OOO 알러지' 형식으로 작성 (예: 복숭아 알러지, 고양이 알러지)
        3. 만약 질환이나 알러지가 없다면, 해당 카테고리에 맞는 일반적인 예방 수칙을 제공해 주세요.
        4. 'integrated_point': 질환과 알레르기를 통합하여 관리해야 할 핵심 포인트를 작성하세요.
    ...
    def _build_profile_prompt(self, health_data: dict, lifestyle: dict, rag_context: str) -> str:
        ...
        [작성 지침]
        1. 'health_guides': '운동', '식단', '수면', '흡연/음주' 4가지 카테고리를 반드시 포함하여 각각 2~3개의 팁을 작성하세요.
        2. 최근 혈압과 최근 혈당이 업데이트 되면, 매번 그에 맞는 가이드를 제공해주세요.
        2. 카드 형태의 UI에 표시될 것이므로 간결하고 실천 가능한 친절한 조언을 문장 형태로 제공하세요.

        [응답 형식]
        {{
            "title": "오늘의 건강 관리 수칙",
            "health_guides": [
                {{ "name": "운동", "tips": ["조깅 30분으로 하루 활력을 채우세요."] }},
                {{ "name": "식단", "tips": ["저염식 실천을 추천드려요."] }},
                {{ "name": "수면", "tips": ["규칙적인 수면을 권장합니다."] }},
                {{ "name": "흡연/음주", "tips": ["금연을 권장합니다."] }}
            ]
        }}
        """.strip()
