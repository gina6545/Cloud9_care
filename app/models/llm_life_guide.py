...
class LLMLifeGuide(models.Model):
    ...
    user: fields.ForeignKeyRelation["User"] = fields.ForeignKeyField(
        "models.User", related_name="life_guides", index=True
    )
    # 생성 시점의 환자 상태(기저질환+알러지+현재약물) 요약 (RAG Context)
    user_current_status = fields.TextField()

    # 모듈화된 가이드 섹션들
    medication_guide: dict | None = fields.JSONField(null=True)  # section1: 복약 및 알레르기
    disease_guide: dict | None = fields.JSONField(null=True)  # section2: 기저질환 지침
    profile_guide: dict | None = fields.JSONField(null=True)  # section3: 생활 습관 (혈압/혈당 포함)
